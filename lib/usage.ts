...
export async function canUserSendBot(userId: string) {
    ...
    const limit = PLAN_LIMITS[user.currentPlan]
    if (!limit) {
        console.error(`❌ Unknown plan: ${user.currentPlan}`)
        return { allowed: false, reason: 'Invalid subscription plan' }
    }
    if (limit.meetings !== -1 && user.meetingsThisMonth >= limit.meetings) {
        return { allowed: false, reason: `You've reached your monthly limit of ${limit.meetings} meetings` }
    }
    return { allowed: true }
}